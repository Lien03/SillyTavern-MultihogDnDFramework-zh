...
const MAPS_GUIDE_HTML = `
    <div style="padding:24px;max-width:760px;max-height:calc(85vh - 48px);box-sizing:border-box;overflow-y:auto;text-align:left;font-family:var(--rt-font,system-ui,sans-serif);color:var(--SmartThemeBodyColor,inherit);line-height:1.55;">
        ...
        <p>地图是 JSON 中持久的“物理”位置，由专门的 Map Architect 智能体生成，并在你身处地图内的每个回合由 Map Updater 更新。之后（我推荐这样做，因为它能让地图真正鲜活起来！）还可由 Map Evolution 智能体按可调间隔定期演化。</p>
        <h3 style="margin:18px 0 6px;color:#bae6fd;">为什么它们很重要？</h3>
        <p>它们之所以重要，尤其在于与技能检定的交互方式。例如，如果没有底层的地图化现实，GM 可能会给你这样的选择：“搜索陷阱 DC 12”或“看看能否发现敌人 DC 8”。如果你选择这样的选项并投出成功，就会找到敌人；如果投出失败，就没有敌人。</p>
        <p>你看出问题了吗？<strong>陷阱或敌人是由掷骰<em>生成/创造</em>的。</strong>另一方面，如果存在底层现实（地图），检定实际上是与物理现实进行比较。这是一种双重上下文系统。</p>
        <p>它还能让一切在空间上保持一致。例如，如果你把天花板压塌在一群敌人身上，Map Updater 会把通道标记为封闭、把敌群标记为已摧毁。道路确实被堵死了，你需要另寻他路。GM 不会凭空捏造无穷无尽的路线。</p>
        <p>这也意味着你可以离开后再回来，坍塌的通道依然在那里等着清理。它让一切非常真实且沉浸。</p>
        <p>更棒的是，当你离开时，Map Evolution 可能会运行，把类似敌对冒险者小队的事物带到同一条通道，他们可能开始替你清理通道。他们可能会用炸药炸开它，而你回来时通道已经畅通！通道被打开的原因由 Map Evolution 记录，因此 GM 知道通道为何是通的。而且你很可能在通道里遇到那支冒险者小队，这将构成一场非常有趣的遭遇。“等等，你是替我打通了那条坍塌的通道<em>吗？</em>”</p>
        <h3 style="margin:18px 0 6px;color:#bae6fd;">如何创建并注入上下文？</h3>
        <p>GM 会为城镇或城市创建街区规模的 SETTLEMENT 地图，为重要的危险建筑群创建房间规模的 DUNGEON 地图，或为重要的低风险多房间场所创建房间规模的 INTERIOR 地图。普通的商店、旅店、礼拜堂和住宅是其聚落街区上的 BUILDING 资产；道具和布景是 OBJECT 资产。荒野和无足轻重的地方不会被地图化。</p>
        <p>聚落首次创建时可以吸收现有的 DUNGEON 或 INTERIOR 地图。GM 也可以从任何地方附加新的 DUNGEON 或 INTERIOR，只需指定一个确切的现有父地图及其一个 AREA 单元格即可；不需要 BUILDING 或玩家移动。SETTLEMENT、DUNGEON 和 INTERIOR 地图最多可容纳三个地图化层级的子级同级地图。CreateAreaMap 拥有这些 SUBDUNGEON/SUBINTERIOR 网关；Map Updater 和 Map Evolution 从不猜测或编辑它们。</p>
        <p>最深处的完整 footer 路径会被激活。相似的名称仍保持区分——“Cellar Crypt”不会匹配“Cellar Crypt Dungeon”——而离开同级地图会重新激活其直接父地图。</p>
        <p>你也可以在 Locations 标题栏中通过“+ Add Mapped Location”按钮手动创建地图，或在任意 Location 词条标题上按“+ MAP”按钮。</p>
        <h3 style="margin:18px 0 6px;color:#bae6fd;">编辑地图</h3>
        <p>你可以直接编辑 JSON 对象来修改地图。打开一张地图，然后点击“&lt;/&gt; Raw JSON”。我之后会添加一个合适的图形化地图编辑器。目前这是唯一的方式。</p>
        <h3 style="margin:18px 0 6px;color:#bae6fd;">世界进程与地图演化的交互</h3>
        <p>Map Evolution 会看到 World Progression 条目。例如，如果地点是 Moonbrook，且 World Progression 产生了一个事件“Moonbrook 正遭受哥布林入侵”，Map Evolution 会将其视为输入，并可能在那里生成哥布林开始大肆破坏。GM 也会看到 World Report，因此即使你不在该地图化地点内，它也能全局得知正在发生的事。这些系统协同工作。</p>
    </div>`;
...
async function populateMapCreationContextOptions(root, ctx) {
    ...
    try {
        ...
        if (!names.length) {
            lorebooks.textContent = '未找到任何 Lorebook。';
        } else {
        ...
        lorebooks.textContent = '无法加载 Lorebook。';
    }
...
async function promptAndRunLorebookAgentMap(siteRoot, locationContent, escapeHtml, { runMapArchitect, inferMapArchitectArgs, listMappedEvolutionSites, lookbackDefault } = {}) {
    ...
    if (!site) return { ok: false, error: '没有可地图化的地点根。' };
    const ctx = SillyTavern.getContext();
    const { Popup } = ctx || {};
    if (!Popup?.show?.confirm) return { ok: false, error: 'Popup API 不可用。' };
    ...
    const includeOptions = eligibleIncludes.length
        ...
        : '<span style="opacity:.6">没有符合条件的 DUNGEON/INTERIOR 同级地图。</span>';
    const html = `
        <div style="text-align:left;font-size:0.9em;line-height:1.4;">
            <p>使用 Map Architect 为 <b>${escapeHtml(site)}</b> 创建私有地图。叙述者不参与。</p>
            <div style="margin-top:10px;display:flex;flex-wrap:wrap;gap:10px 16px;align-items:center;">
                <label style="display:flex;gap:6px;align-items:center;"><input type="radio" name="rt-map-create-mode" value="auto" checked onchange="var a=document.getElementById('rt-map-create-auto');var m=document.getElementById('rt-map-create-manual');if(a)a.hidden=false;if(m)m.hidden=true"> 自动</label>
                <label style="display:flex;gap:6px;align-items:center;"><input type="radio" name="rt-map-create-mode" value="manual" onchange="var a=document.getElementById('rt-map-create-auto');var m=document.getElementById('rt-map-create-manual');if(a)a.hidden=true;if(m)m.hidden=false"> 手动</label>
            </div>
            <div id="rt-map-create-auto" style="margin-top:8px;">
                <p style="margin:0 0 6px;">Map Architect 会根据该地点的传说与近期故事填充入口、种类、规模、威胁、详细的生成提示和简要描述，然后生成地图。</p>
                <label style="display:block;">故事回溯
                ...
                <p style="margin:4px 0 0;opacity:0.65;font-size:0.85em;">0 表示不使用最近的聊天记录。</p>
            </div>
            <div id="rt-map-create-manual" hidden>
                <div style="margin-top:10px;display:flex;flex-wrap:wrap;gap:10px 16px;align-items:center;">
                    <label style="display:flex;gap:6px;align-items:center;"><input type="radio" name="rt-map-create-kind" value="SETTLEMENT" checked onchange="var t=document.getElementById('rt-map-create-threat');if(t)t.value='MODERATE'"> 聚落</label>
                    <label style="display:flex;gap:6px;align-items:center;"><input type="radio" name="rt-map-create-kind" value="DUNGEON" onchange="var t=document.getElementById('rt-map-create-threat');if(t)t.value='HIGH'"> 地下城</label>
                    <label style="display:flex;gap:6px;align-items:center;"><input type="radio" name="rt-map-create-kind" value="INTERIOR" onchange="var t=document.getElementById('rt-map-create-threat');if(t)t.value='LOW'"> 室内</label>
                </div>
                <label style="display:block;margin-top:8px;">入口
                    <input id="rt-map-create-entrance" type="text" value="Entrance" style="width:100%;margin-top:3px;box-sizing:border-box;">
                </label>
                <div style="display:flex;gap:8px;margin-top:8px;">
                    <label style="flex:1;">规模
                ...
                <label style="display:block;margin-top:8px;">地图生成提示
                ...
                <label style="display:block;margin-top:8px;">简要描述
                ...
                <details style="margin-top:8px;">
                    <summary style="cursor:pointer;">吸收现有的同级地图（仅限 SETTLEMENT）</summary>
                    <div style="display:flex;flex-direction:column;gap:4px;margin-top:6px;">${includeOptions}</div>
                </details>
            </div>
            <details style="margin-top:10px;">
                <summary style="cursor:pointer;">可选参考上下文</summary>
                <p style="margin:6px 0;opacity:0.7;font-size:0.85em;">即使故事回溯设为 0，所选来源也会发送给 Map Architect。</p>
                <label style="display:block;font-weight:600;">Lorebook</label>
                <div data-map-context-lorebooks style="max-height:120px;overflow-y:auto;border:1px solid rgba(255,255,255,0.14);border-radius:4px;padding:5px;margin-top:3px;">正在加载 Lorebook…</div>
                <label style="display:block;font-weight:600;margin-top:8px;">角色卡</label>
    ...
    let form = null;
    const choice = await Popup.show.confirm('创建地图', html, {
        okButton: '生成地图',
        cancelButton: '取消',
        onClosing: (popup) => {
            if (popup.result) {
    ...
    if (!form) return { ok: false, error: '无法读取地图表单。' };
    ...
    try {
        ...
        if (typeof inferMapArchitectArgs !== 'function') {
            return { ok: false, error: 'Map Architect 自动填充不可用。' };
        }
        try { globalThis.toastr?.info?.(`正在为 ${site} 填充地图简报…`, 'Map Architect', { timeOut: 4000 }); } catch (_) { /* best effort */ }
...
async function promptAndCreateMappedLocation({ runMapArchitect, inferMapArchitectArgs, listMappedEvolutionSites, escapeHtml, lookbackDefault } = {}) {
    ...
    const html = `
        <div style="text-align:left;font-size:0.9em;line-height:1.4;">
            <p>创建新的地点根并生成其私有地图。叙述者不参与。</p>
            <label style="display:block;margin-top:8px;">地点名称
                <input id="rt-map-loc-name" type="text" placeholder="精确地点根，不要包含 ::" style="width:100%;margin-top:3px;box-sizing:border-box;">
            </label>
            <div style="margin-top:10px;display:flex;flex-wrap:wrap;gap:10px 16px;align-items:center;">
                <label style="display:flex;gap:6px;align-items:center;"><input type="radio" name="rt-map-loc-mode" value="auto" checked onchange="var a=document.getElementById('rt-map-loc-auto');var m=document.getElementById('rt-map-loc-manual');if(a)a.hidden=false;if(m)m.hidden=true"> 自动</label>
                <label style="display:flex;gap:6px;align-items:center;"><input type="radio" name="rt-map-loc-mode" value="manual" onchange="var a=document.getElementById('rt-map-loc-auto');var m=document.getElementById('rt-map-loc-manual');if(a)a.hidden=true;if(m)m.hidden=false"> 手动</label>
            </div>
            <div id="rt-map-loc-auto" style="margin-top:8px;">
                <label style="display:block;">简报
                    <textarea id="rt-map-loc-brief" placeholder="这是什么地方。其余由 Map Architect 填充。" style="width:100%;height:88px;margin-top:3px;box-sizing:border-box;resize:vertical;"></textarea>
                </label>
                <label style="display:block;margin-top:8px;">故事回溯
                ...
                <p style="margin:4px 0 0;opacity:0.65;font-size:0.85em;">0 表示不使用最近的聊天记录——仅使用名称和简报。</p>
            </div>
            <div id="rt-map-loc-manual" hidden>
                <label style="display:block;margin-top:8px;">额外关键词
                    <input id="rt-map-loc-keys" type="text" placeholder="可选别名，逗号分隔" style="width:100%;margin-top:3px;box-sizing:border-box;">
                </label>
                <p style="margin:4px 0 0;opacity:0.65;font-size:0.85em;">地点名称会自动添加。最多 6 个。</p>
                <div style="margin-top:10px;display:flex;flex-wrap:wrap;gap:10px 16px;align-items:center;">
                    <label style="display:flex;gap:6px;align-items:center;"><input type="radio" name="rt-map-loc-kind" value="SETTLEMENT" checked onchange="var t=document.getElementById('rt-map-loc-threat');if(t)t.value='MODERATE'"> 聚落</label>
                    <label style="display:flex;gap:6px;align-items:center;"><input type="radio" name="rt-map-loc-kind" value="DUNGEON" onchange="var t=document.getElementById('rt-map-loc-threat');if(t)t.value='HIGH'"> 地下城</label>
                    <label style="display:flex;gap:6px;align-items:center;"><input type="radio" name="rt-map-loc-kind" value="INTERIOR" onchange="var t=document.getElementById('rt-map-loc-threat');if(t)t.value='LOW'"> 室内</label>
                </div>
                <label style="display:block;margin-top:8px;">入口
                ...
                <div style="display:flex;gap:8px;margin-top:8px;">
                 ...
                 <label style="display:block;margin-top:8px;">地图生成提示
                    <textarea id="rt-map-loc-prompt" placeholder="为地图架构师提供的详细私有事实与创意指导" style="width:100%;height:110px;margin-top:3px;box-sizing:border-box;resize:vertical;"></textarea>
                 </label>
                 <label style="display:block;margin-top:8px;">简要描述
                    <input id="rt-map-loc-brief-description" placeholder="提供给 CORE 与地图网关的当前简要描述" style="width:100%;margin-top:3px;box-sizing:border-box;">
                 </label>
                 <details style="margin-top:8px;">
                     <summary style="cursor:pointer;">吸收现有已映射同类地点（仅限定居点）</summary>
                     <div style="display:flex;flex-direction:column;gap:4px;margin-top:6px;">${includeOptions}</div>
                 </details>
            </div>
            <details style="margin-top:10px;">
                <summary style="cursor:pointer;">可选参考上下文</summary>
                <p style="margin:6px 0;opacity:0.7;font-size:0.85em;">即使故事回溯设为 0，所选来源也会发送给地图架构师。</p>
                <label style="display:block;font-weight:600;">世界书</label>
                <div data-map-context-lorebooks style="max-height:120px;overflow-y:auto;border:1px solid rgba(255,255,255,0.14);border-radius:4px;padding:5px;margin-top:3px;">正在加载世界书…</div>
                <label style="display:block;font-weight:600;margin-top:8px;">角色卡</label>
                <div data-map-context-character-cards style="max-height:120px;overflow-y:auto;border:1px solid rgba(255,255,255,0.14);border-radius:4px;padding:5px;margin-top:3px;"></div>
            </details>
        </div>
    ...
    let form = null;
    const choice = await Popup.show.confirm('添加已映射地点', html, {
        okButton: '创建地点和地图',
        cancelButton: '取消',
        onClosing: (popup) => {
            if (popup.result) {
    ...
    if (!choice) return { ok: false, cancelled: true };
    if (!form) return { ok: false, error: '无法读取已映射地点表单。' };

    const site = form.site;
    if (!site) return { ok: false, error: '地点名称是必填项。' };
    if (site.includes('::')) return { ok: false, error: '请使用地点根名称，而不是嵌套的 :: 路径。' };

    try {
        if (form.mode === 'manual') {
            if (!form.prompt || !form.briefDescription) return { ok: false, error: '地图生成提示和简要描述是必填项。' };
            await runMapArchitect({
                site,
                entrance: form.entrance,
                kind: form.kind,
        ...
        if (typeof inferMapArchitectArgs !== 'function') {
            return { ok: false, error: '地图架构师自动填充不可用。' };
        }
        try { globalThis.toastr?.info?.(`正在为 ${site} 填充地图简报…`, '地图架构师', { timeOut: 4000 }); } catch (_) { /* best effort */ }
        const inferred = await inferMapArchitectArgs({
            site,
...
async function promptMappedEvolutionSites(sites, escapeHtml) {
    ...
    body.innerHTML = `
        <div style="font-size:13px; line-height:1.4; opacity:0.85;">
            选择现在要演化的已映射地点。将跳过间隔到期检查。
        </div>
        <div class="flex-container gap-1" style="flex-wrap:wrap;">
            <button type="button" class="menu_button" data-evo-pick="all">全部</button>
            <button type="button" class="menu_button" data-evo-pick="none">无</button>
            <button type="button" class="menu_button" data-evo-pick="current">当前</button>
        </div>
        <div data-evo-pick-list style="max-height:240px; overflow-y:auto; border:1px solid rgba(255,255,255,0.12); border-radius:6px; padding:6px;">
            ${rows.map(site => {
                ...
                const suffix = site.current ? ' <small>（当前）</small>' : '';
...
export function createPanel(dependencies) {
    ...
    if (agentPanel && agentCloseBtn) {
        ...
        const buildEntryBody = (item, entryHdr, opts = {}) => {
            ...
            staleBadge.textContent = '⚠ 条目已在外部更改 — 保存将丢弃外部更改，或取消以重新加载。';
            ...
            const syncReadFromItem = () => {
                ...
                if (!isNpcEntry && coreMatch) {
                    ...
                    coreRead.innerHTML = `<div class="rt-agent-core-label">永久</div><div class="rt-agent-core-text">${escapeHtmlWithColor(substituteDisplayMacros(coreMatch[1].trim()))}</div>`;
                } else {
                    coreRead.style.display = 'none';
                    coreRead.innerHTML = '';
                }
                ...
                if (isNpcEntry) {
                    contentRead.textContent = substituteDisplayMacros(dynamic) || '（尚未记录战役历史）';
                    ...
                    contentRead.textContent = substituteDisplayMacros(raw) || '（空）';
                    contentRead.style.display = 'block';
                }
            };
            syncReadFromItem();
            ...
            cancelBtn.title = '关闭编辑器并放弃未保存的更改';
            ...
            if (cleanBtn) {
                cleanBtn.addEventListener('click', async (e) => {
                    ...
                    if (isRouterRunning()) {
                        // @ts-ignore
                        toastr.warning('Agent 正在运行中。', 'Lorebook Agent');
                        return;
                    }
                    ...
                    const promptHtml = `
                            <div style="text-align: left; font-size: 0.9em; line-height: 1.4;">
                                <p>您正在对 <b>${escapeHtml(item.label)}</b> 触发一次定向清理。</p>
                                <p style="margin-top: 8px;">为此条目的压缩输入自定义要求（例如 <i>"保留性格部分完整"</i> 或 <i>"缩短为 3 条简洁要点"</i>）：</p>
                                <textarea id="rt-entry-clean-instructions" style="width: 100%; height: 60px; background: rgba(0,0,0,0.3); border: 1px solid rgba(255,255,255,0.2); color: white; border-radius: 4px; padding: 5px; font-size: 12px; box-sizing: border-box; resize: none; margin-top: 5px;" placeholder="留空以使用标准压缩……"></textarea>
                            </div>
                        `;

                    const choice = await Popup.show.confirm('🧹 定向条目清理', promptHtml, {
                        okButton: '清理条目',
                        cancelButton: '取消'
                    });
            ...
            saveBtn.addEventListener('click', async (e) => {
                ...
                if (isRouterRunning()) {
                    // @ts-ignore
                    toastr.warning('Agent 正在运行——请等待其完成后再保存。', 'Lorebook Agent');
                    return;
                }
                ...
                if (ok) {
                    ...
                    // @ts-ignore
                    toastr.success('条目已保存。', 'Lorebook Agent');
                } else {
                    saveBtn.textContent = '保存';
                    saveBtn.disabled = false;
                    // @ts-ignore
                    toastr.error('保存失败。', 'Lorebook Agent');
                }
            });
            ...
            if (delBtn) {
                delBtn.addEventListener('click', async (e) => {
                    e.stopPropagation();
                    if (confirm(`确定删除条目“${item.label}”吗？`)) {
                        ...
                        if (ok) {
                            ...
                            await refreshManifest();
                            // @ts-ignore
                            toastr.success(`已删除“${item.label}”`, 'Lorebook Agent');
                        }
                    }
                });
            }
                ...
                pinBtn.addEventListener('click', async (e) => {
                    ...
                    if (ok) {
                        ...
                        toastr.info(nextPinned
                            ? `已固定“${item.label}”——始终生效`
                            : `已取消固定“${item.label}”`, 'Lorebook Agent');
                    }
                });
            }
        ...
        const saveCampaignNpcToLibrary = async (item) => {
            ...
            if (!name) {
                toastr['warning']('此卡片没有可保存的名称。', 'Library');
                return;
            }
            const s = getSettings();
            const existing = findLibraryNpcByName(s, name);
            if (existing && !confirm(`“${name}”已在库中。要覆盖它吗？`)) return;
            try {
                ...
                toastr['success'](`已将“${name}”保存到库中。`, 'Library');
            } catch (err) {
                toastr['error'](`保存卡片失败：${String(err.message || err).substring(0, 120)}`, 'Library');
            }
        };
        ...
        const renderSectionsHtml = (rawContent, isPC = false, options = {}) => {
            ...
            if (!options.omitDynamic && parsed.dynamic.length > 0) {
                html += `<div style="font-size:11px;font-weight:bold;color:rgba(255,255,255,0.4);text-transform:uppercase;letter-spacing:1.5px;margin-top:24px;margin-bottom:14px;border-bottom:1px solid rgba(255,255,255,0.1);padding-bottom:4px;">📖 战役历史与动态世界设定</div>`;
        ...
        const openLibraryNpcCard = async (record) => {
            ...
            const sectionsInitialHtml = renderSectionsHtml(record.content, false, { omitDynamic: true })
                || '<div style="font-size:14px;color:var(--SmartThemeBodyColor, inherit);opacity:0.5;font-style:italic;padding:16px 0;">未找到结构化段落。点击「编辑文本」以添加内容。</div>';
            const renderLibraryPopupPortraitInner = (src) => {
                ...
                return `${imgOrPlaceholder}<div class="rt-npc-portrait-gen-overlay" title="${src ? '更换立绘' : '设置立绘'}" style="font-size:32px;">${src ? '⚙️' : '🎨'}</div>`;
            };
            ...
            popupDom.innerHTML = `
                <div style="display:flex;gap:24px;margin-bottom:20px;align-items:flex-start;flex-wrap:wrap;">
                    ...
                    <div style="flex:1;min-width:220px;display:flex;flex-direction:column;gap:8px;">
                        <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px;">
                            <div style="font-size:24px;font-weight:bold;color:#d4a940;line-height:1.2;">${escapeHtml(record.name || '未命名NPC')}</div>
                        ...
                        <span style="font-size:11px;padding:3px 10px;border-radius:10px;font-weight:bold;align-self:flex-start;background:rgba(212,169,64,0.12);color:#d4a940;border:1px solid rgba(212,169,64,0.35);">📚 资料库</span>
                        ${keysLabel ? `<div style="font-size:12px;opacity:0.65;">关键词：${escapeHtml(keysLabel)}</div>` : ''}
                    </div>
                </div>
                <div style="border-top:2px solid rgba(212,169,64,0.15);padding-top:18px;">
                    ...
                    <div class="rt-npc-popup-edit" style="display:none;flex-direction:column;gap:10px;">
                        <div style="font-size:11px;font-weight:bold;color:rgba(255,255,255,0.4);text-transform:uppercase;letter-spacing:1.5px;margin-bottom:4px;">✏️ 编辑资料库条目</div>
            ...
            saveBtn?.addEventListener('click', async () => {
                ...
                try {
                    ...
                    editPane.style.display = 'none';
                    viewPane.style.display = 'block';
                    toastr['success'](`已在NPC资料库中更新“${record.name}”。`, 'NPC资料库');
                    document.dispatchEvent(new CustomEvent('rt_npc_library_updated'));
                } catch (err) {
                    toastr['error'](`保存失败：${String(err.message || err).substring(0, 120)}`, 'NPC资料库');
                } finally {
                    saveBtn.disabled = false;
                    saveBtn.textContent = '💾 保存';
                }
        ...
        const performManifestRefresh = async (source = 'auto') => {
            ...
            try {
                ...
                if (runtimeState.currentChatId && s.chatStates?.[runtimeState.currentChatId]?.playerCharacter) {
                    ...
                    pcDiv.innerHTML = `
                        <div class="rt-npc-portrait-wrap">
                            ${portraitHtml}
                            <div class="rt-npc-portrait-gen-overlay" title="${portraitSrc ? '管理立绘' : '生成立绘'}">${portraitSrc ? '⚙️' : '🎨'}</div>
                        </div>
                        <div class="rt-npc-info">
                            ...
                            <span class="rt-npc-status-badge active" style="background:rgba(120,80,220,0.15);color:#c4b5fd;border:1px solid rgba(120,80,220,0.5);">👤 玩家角色</span>
                            <div class="rt-npc-actions">
                                <button class="rt-npc-action-btn rt-npc-view" title="查看PC卡"><i class="fa-solid fa-address-card"></i> 完整PC卡</button>
                                <button class="rt-npc-action-btn rt-npc-library" title="保存到资料库"><i class="fa-solid fa-bookmark"></i></button>
                                <button class="rt-npc-action-btn rt-npc-edit" title="编辑PC文本"><i class="fa-solid fa-pen-to-square"></i></button>
                                <button class="rt-npc-action-btn rt-npc-delete" title="解除玩家角色关联"><i class="fa-solid fa-link-slash"></i></button>
                            </div>
                        </div>
                    `;
                    ...
                    const openPcPopup = async (startInEditMode = false) => {
                        ...
                        const sectionsInitialHtml = renderSectionsHtml(pc.bio, true) || '<div style="font-size:14px;color:var(--SmartThemeBodyColor, inherit);opacity:0.5;font-style:italic;padding:16px 0;">未找到结构化段落。</div>';
                        ...
                        popupDom.innerHTML = `
                                ...
                                <div style="flex:1;min-width:220px;display:flex;flex-direction:column;gap:8px;">
                                    <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px;">
                                        ...
                                        <div style="display:flex;flex-direction:column;gap:4px;align-items:flex-end;">
                                            <button class="rt-npc-popup-edit-btn menu_button" style="flex-shrink:0;font-size:12px;padding:4px 12px;white-space:nowrap;">✏️ 编辑文本</button>
                                            <button class="rt-npc-popup-ai-edit-btn menu_button" style="flex-shrink:0;font-size:12px;padding:4px 12px;white-space:nowrap;background:rgba(120,80,220,0.15);border-color:rgba(120,80,220,0.5);color:#c4b5fd;">✨ AI编辑</button>
                                            <button class="rt-npc-popup-library-btn menu_button" style="flex-shrink:0;font-size:12px;padding:4px 12px;white-space:nowrap;">📚 保存到资料库</button>
                                        </div>
                                    </div>
                            ...
                            <div style="border-top:2px solid rgba(120,80,220,0.2);padding-top:18px;">
                                ...
                                <div class="rt-npc-popup-ai-edit" style="display:none;flex-direction:column;gap:10px;">
                                    <div style="font-size:11px;font-weight:bold;color:rgba(120,80,220,0.9);text-transform:uppercase;letter-spacing:1.5px;margin-bottom:4px;">✨ AI编辑请求</div>
                                    <textarea class="rt-npc-popup-ai-instructions" spellcheck="true" placeholder="描述你想要修改或迭代的内容……例如：「让背景与正在进行的战争联系更紧密」或「让她更愤世嫉俗」" style="width:100%;min-height:90px;box-sizing:border-box;background:var(--SmartThemeBlurTintColor, rgba(0,0,0,0.3));color:var(--SmartThemeBodyColor, inherit);border:1px solid rgba(120,80,220,0.5);border-radius:8px;padding:12px;font-family:inherit;font-size:13px;line-height:1.6;resize:vertical;"></textarea>
                                    <div style="display:flex;gap:8px;justify-content:flex-end;">
                                    ...
                                    <div class="rt-npc-popup-ai-preview" style="display:none;flex-direction:column;gap:8px;">
                                        <div style="font-size:11px;font-weight:bold;color:rgba(255,255,255,0.4);text-transform:uppercase;letter-spacing:1.5px;">预览 — 应用前请检查</div>
                        ...
                        saveBtn.addEventListener('click', async () => {
                            ...
                            const newHtml = renderSectionsHtml(pc.bio, true) || `<div style="font-size:14px;color:var(--SmartThemeBodyColor, inherit);opacity:0.5;font-style:italic;padding:16px 0;">未找到结构化段落。</div>`;
                            sectionsDiv.innerHTML = newHtml;
                            showPane(viewPane);
                            if (typeof refreshAgentManifestNow === 'function') refreshAgentManifestNow();
                            // @ts-ignore
                            if (typeof toastr !== 'undefined') toastr.success('玩家角色已保存。', '战役记录');
                        });
                        ...
                        const runAiEdit = async () => {
                            ...
                            if (!instructions) { toastr['warning']('请描述你想要修改的内容。', 'AI编辑'); return; }
                            ...
                            try {
                                ...
                                if (trimmed) {
                                    ...
                                    toastr['warning']('AI返回了空结果，请重试。', 'AI编辑');
                                }
                            } catch (err) {
                                toastr['error'](`AI编辑失败：${String(err.message || err).substring(0, 120)}`, 'AI编辑');
                            }
                        ...
                        aiApplyBtn.addEventListener('click', async () => {
                            ...
                            if (typeof refreshAgentManifestNow === 'function') refreshAgentManifestNow();
                            // @ts-ignore
                            if (typeof toastr !== 'undefined') toastr.success('已通过 AI 更新玩家角色。', '战役记录');
                        });
                ...
                const lorebookRenderTask = async () => {
                    try {
                        ...
                        for (const [bookName, items] of byBook) {
                            ...
                            ${isNpcBook ? '<button type="button" class="rt-npc-manager-btn" title="NPC/PC 管理器 — 图库、添加到故事、导入/导出"><i class="fa-solid fa-users-gear"></i> NPC/PC 管理器</button>' : ''}
                            ${isNpcBook ? '<button class="rt-npc-settings-btn" title="NPC 与 PC 卡设置" style="background:none;border:none;cursor:pointer;font-size:11px;opacity:0.5;padding:0;margin:0;width:14px;height:14px;display:inline-flex;align-items:center;justify-content:center;color:var(--rt-text-muted);flex-shrink:0;line-height:1;" onclick="event.stopPropagation()">⚙️</button>' : ''}
                            ${isLocBook ? '<button type="button" class="rt-loc-maps-guide-btn" title="了解持久地图的工作方式"><i class="fa-solid fa-circle-info"></i> 地图指南</button>' : ''}
                            ${isLocBook && dungeonRealityEnabled ? '<button type="button" class="rt-loc-add-mapped-btn" title="创建新的地点根节点并生成其专属地图"><i class="fa-solid fa-plus"></i> 添加映射地点</button>' : ''}
                            ${isLocBook ? '<button class="rt-loc-settings-btn" title="地点设置" style="background:none;border:none;cursor:pointer;font-size:11px;opacity:0.5;padding:0;margin:0;width:14px;height:14px;display:inline-flex;align-items:center;justify-content:center;color:var(--rt-text-muted);flex-shrink:0;line-height:1;" onclick="event.stopPropagation()">⚙️</button>' : ''}
                        `;
                            ...
                            if (isNpcBook) {
                                ...
                                if (settingsBtn) {
                                    ...
                                    <div style="font-size:16px;font-weight:bold;color:#d4a940;margin-bottom:16px;">⚙️ NPC 与 PC 卡设置</div>

                                    <div style="margin-bottom:6px;display:flex;align-items:center;gap:10px;">
                                        <label style="font-size:12px;color:rgba(255,255,255,0.7);flex:1;">显示 NPC 头像</label>
                                        <label style="display:flex;align-items:center;gap:6px;cursor:pointer;">
                                            ...
                                            <span style="font-size:11px;color:rgba(255,255,255,0.5);">${curS.npcPortraits !== false ? '已启用' : '已禁用'}</span>
                                        </label>
                                    </div>
                                    <div style="font-size:10px;color:rgba(255,255,255,0.35);margin-bottom:14px;">禁用后，NPC 将使用紧凑列表视图（类似事件/地点），并关闭 NPC 头像自动生成。</div>

                                    <div style="margin-bottom:14px;">
                                        <label style="font-size:12px;color:rgba(255,255,255,0.7);display:block;margin-bottom:4px;">主要 NPC 总字数目标</label>
                                        ...
                                        <div style="font-size:10px;color:rgba(255,255,255,0.35);margin-top:2px;">反复出现、对剧情重要的 NPC。默认：全部分区合计 225 词</div>
                                    </div>

                                    <div style="margin-bottom:14px;">
                                        <label style="font-size:12px;color:rgba(255,255,255,0.7);display:block;margin-bottom:4px;">次要 NPC 总字数目标</label>
                                        <input type="number" id="rt-npc-minor-words" value="${curS.npcMinorWords ?? 135}" min="1" max="5000" step="5"
                                            style="width:100%;background:rgba(0,0,0,0.4);color:white;border:1px solid rgba(255,255,255,0.15);border-radius:6px;padding:6px 10px;font-size:13px;box-sizing:border-box;">
                                        <div style="font-size:10px;color:rgba(255,255,255,0.35);margin-top:2px;">店主、守卫、一次性遭遇。默认：全部分区合计 135 词</div>
                                    </div>

                                    <div style="margin-bottom:6px;display:flex;align-items:center;gap:10px;">
                                        <label style="font-size:12px;color:rgba(255,255,255,0.7);flex:1;">关系系统</label>
                                        <label style="display:flex;align-items:center;gap:6px;cursor:pointer;">
                                            <input type="checkbox" id="rt-npc-rel-bars" ${curS.npcRelationshipBars ? 'checked' : ''}
                                                style="width:16px;height:16px;accent-color:#d4a940;cursor:pointer;">
                                            <span style="font-size:11px;color:rgba(255,255,255,0.5);">${curS.npcRelationshipBars ? '已启用' : '已禁用'}</span>
                                        </label>
                                    </div>
                                    <div style="font-size:10px;color:rgba(255,255,255,0.35);margin-bottom:14px;">在 NPC 卡片和弹窗上显示友谊/好感度跟踪条，同时向 AI 指令添加关系字段。</div>

                                    <div style="margin-bottom:6px;display:flex;align-items:center;gap:10px;">
                                        <label style="font-size:12px;color:rgba(255,255,255,0.7);flex:1;">显示关系浮动反馈</label>
                                        <label style="display:flex;align-items:center;gap:6px;cursor:pointer;">
                                            <input type="checkbox" id="rt-npc-rel-toast" ${curS.npcRelationshipToast !== false ? 'checked' : ''}
                                                style="width:16px;height:16px;accent-color:#d4a940;cursor:pointer;">
                                            <span style="font-size:11px;color:rgba(255,255,255,0.5);">${curS.npcRelationshipToast !== false ? '已启用' : '已禁用'}</span>
                                        </label>
                                    </div>
                                    <div style="font-size:10px;color:rgba(255,255,255,0.35);margin-bottom:14px;">当友谊/好感度数值变化时，显示一个上升并淡出的浮动图形。</div>

                                    <div style="margin-bottom:14px;">
                                        <label style="font-size:12px;color:rgba(255,255,255,0.7);display:block;margin-bottom:4px;">“按原样添加”导入模式</label>
                                        <div style="display:flex;flex-direction:column;gap:5px;">
                                            <label style="display:flex;align-items:flex-start;gap:8px;cursor:pointer;" title="将卡片的原始内容按原样包裹在 [CORE][/CORE] 标签中。不涉及 AI。">
                                                ...
                                                <span style="font-size:11px;color:rgba(255,255,255,0.75);"><b>原样</b> — 将卡片逐字包裹在 [CORE][/CORE] 中。不涉及 AI。</span>
                                            </label>
                                            <label style="display:flex;align-items:flex-start;gap:8px;cursor:pointer;" title="将卡片发送给 AI 进行最小限度的逻辑审查。仅修正世界/时代冲突，尽可能完整保留原文。">
                                                <input type="radio" name="rt-npc-add-as-is-mode" value="ai_review" ${(curS.npcAddAsIsMode ?? 'ai_review') === 'ai_review' ? 'checked' : ''}
                                                    style="margin-top:3px;accent-color:#d4a940;cursor:pointer;">
                                                <span style="font-size:11px;color:rgba(255,255,255,0.75);"><b>AI 审查</b> — 仅对时代/世界冲突进行最小修正。保留原文。</span>
                                            </label>
                                        </div>
                                        <div style="font-size:10px;color:rgba(255,255,255,0.35);margin-top:4px;">控制点击角色卡片上的“+ 按原样添加”时的行为。</div>
                                    </div>

                                    <div style="margin-bottom:14px;">
                                        <label style="font-size:12px;color:rgba(255,255,255,0.7);display:block;margin-bottom:4px;">关系上限 — 当前聊天（± 范围）</label>
                                        <input type="number" id="rt-npc-rel-max" value="${getNpcRelationshipMax(curS)}" min="10" max="10000" step="10"
                                            style="width:100%;background:rgba(0,0,0,0.4);color:white;border:1px solid rgba(255,255,255,0.15);border-radius:6px;padding:6px 10px;font-size:13px;box-sizing:border-box;">
                                        <div style="font-size:10px;color:rgba(255,255,255,0.35);margin-top:2px;">仅针对当前聊天的剧情规模。友谊/好感度范围为 −N 到 +N。新聊天以扩展设置中的默认值为起点（当前为 ${getNpcRelationshipMaxDefault(curS)}）。</div>
                                    </div>

                                    <div style="margin-bottom:6px;display:flex;align-items:center;gap:10px;">
                                        <label style="font-size:12px;color:rgba(255,255,255,0.7);flex:1;">导入角色卡片时忽略角色限制</label>
                                        <label style="display:flex;align-items:center;gap:6px;cursor:pointer;">
                                            <input type="checkbox" id="rt-ignore-npc-limits" ${curS.ignoreNpcImportLimits ? 'checked' : ''}
                                                style="width:16px;height:16px;accent-color:#d4a940;cursor:pointer;">
                                            <span style="font-size:11px;color:rgba(255,255,255,0.5);">${curS.ignoreNpcImportLimits ? '已启用' : '已禁用'}</span>
                                        </label>
                                    </div>
                                    <div style="font-size:10px;color:rgba(255,255,255,0.35);margin-bottom:10px;">从 NPC 提示词中省略 &lt;CORE LENGTH TARGETS&gt; 部分。</div>
                                    ...
                                    <button id="rt-btn-edit-pc-sections-inline" style="width:100%;background:rgba(180, 100, 255, 0.15);border:1px solid rgba(180, 100, 255, 0.4);color:white;border-radius:6px;padding:8px 10px;font-size:13px;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;margin-bottom:10px;transition:background 0.2s;" title="自定义玩家角色的角色设定分区（例如：外观、性格、特长）。">
                                        ...
                                        setTimeout(() => {
                                            ...
                                            if (relEl) {
                                                relEl.addEventListener('change', () => {
                                                    ...
                                                    if (relEl.nextElementSibling) relEl.nextElementSibling.textContent = newRel ? '已启用' : '已禁用';
                                                });
                                            }
                                            if (ignoreEl) {
                                                ignoreEl.addEventListener('change', () => {
                                                    newIgnoreLimits = ignoreEl.checked;
                                                    if (ignoreEl.nextElementSibling) ignoreEl.nextElementSibling.textContent = newIgnoreLimits ? '已启用' : '已禁用';
                                                });
                                            }
                                            ...
                                            if (relToastEl) {
                                                relToastEl.addEventListener('change', () => {
                                                    ...
                                                    if (relToastEl.nextElementSibling) relToastEl.nextElementSibling.textContent = newRelToast ? '已启用' : '已禁用';
                                                });
                                            }
                                            ...
                                            if (portraitsEl) {
                                                portraitsEl.addEventListener('change', () => {
                                                    ...
                                                    if (portraitsEl.nextElementSibling) portraitsEl.nextElementSibling.textContent = newNpcPortraits ? '已启用' : '已禁用';
                                                });
                                            }
                                        ...
                                        if (result) {
                                            ...
                                            saveSettings();
                                            toastr['success']('NPC 与 PC 卡片设置已保存。', 'NPC 与 PC 卡片设置');
                            ...
                            if (isLocBook) {
                                    ...
                                    addMappedBtn.addEventListener('click', async (e) => {
                                        ...
                                        try {
                                            ...
                                            if (choice?.ok) {
                                                ...
                                                toastr.error(choice?.error || '无法创建该映射位置。', '持久地图');
                                            }
                                        } finally {
                                ...
                                if (locSettingsBtn) {
                                    locSettingsBtn.addEventListener('click', async (e) => {
                                        ...
                                        const curS = getSettings();
                                        const popupHtml = `<div style="padding:16px;width:320px;text-align:left;font-family:var(--rt-font, system-ui, sans-serif);">
                                    <div style="font-size:16px;font-weight:bold;color:#5eb8d4;margin-bottom:16px;">⚙️ 位置设置</div>
                                    <div style="margin-bottom:6px;display:flex;align-items:center;gap:10px;">
                                        <label style="font-size:12px;color:rgba(255,255,255,0.7);flex:1;">显示位置图片</label>
                                        <label style="display:flex;align-items:center;gap:6px;cursor:pointer;">
                                            ...
                                            <span style="font-size:11px;color:rgba(255,255,255,0.5);">${!!curS.locationImages ? '已启用' : '未启用'}</span>
                                        </label>
                                    </div>
                                    <div style="font-size:10px;color:rgba(255,255,255,0.35);">禁用后，位置将使用纯树状列表，不显示缩略图或场景详情弹窗，位置自动生成功能也会关闭。</div>
                                </div>`;
                                        ...
                                        setTimeout(() => {
                                            ...
                                            if (locEl) {
                                                locEl.addEventListener('change', () => {
                                                    ...
                                                    if (locEl.nextElementSibling) locEl.nextElementSibling.textContent = newLocImages ? '已启用' : '未启用';
                                                });
                                            }
                                        }, 0);
                            ...
                            if (isNpcBook) {
                                ...
                                renderCompactRelStats = (entryId) => {
                                    ...
                                    const fmtVal = (val, type) => {
                                        ...
                                        const label = type === 'friendship' ? '友谊' : '好感';
                                ...
                                openNpcDetailPopup = async (item, rel) => {
                                    ...
                                    <div style="display:grid;grid-template-columns:auto 80px 1fr 40px;align-items:center;column-gap:12px;row-gap:6px;">
                                        ...
                                        <input type="number" id="rt-npc-detail-${type}-input" min="-${relMax}" max="${relMax}" step="1" value="${clamped}"
                                            aria-label="${label} 数值" style="width:52px;box-sizing:border-box;font-size:14px;font-weight:bold;text-align:right;color:${valColor};font-family:monospace;background:rgba(0,0,0,.18);border:1px solid var(--SmartThemeBorderColor, rgba(128,128,128,.35));border-radius:4px;padding:2px 4px;">
                                        <div></div>
                                        <div></div>
                            ...
                            `;
                                    ...
                                    const sectionsInitialHtml = renderSectionsHtml(item.content)
                                        || '<div style="font-size:14px;color:var(--SmartThemeBodyColor, inherit);opacity:0.5;font-style:italic;padding:16px 0;">未找到结构化章节，请点击“编辑文本”添加内容。</div>';
                                    ...
                                    const activeLabel = item.is_active ? '● 启用' : '○ 停用';
                                ...
                                <div style="display:flex;gap:24px;margin-bottom:20px;align-items:flex-start;flex-wrap:wrap;">
                                    ...
                                    <div style="flex:1;min-width:220px;display:flex;flex-direction:column;gap:8px;">
                                        <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px;">
                                            ...
                                            <div style="display:flex;flex-direction:column;gap:4px;align-items:flex-end;">
                                                <button class="rt-npc-popup-edit-btn menu_button" style="flex-shrink:0;font-size:12px;padding:4px 12px;white-space:nowrap;">✏️ 编辑文本</button>
                                                <button class="rt-npc-popup-ai-edit-btn menu_button" style="flex-shrink:0;font-size:12px;padding:4px 12px;white-space:nowrap;background:rgba(212,169,64,0.1);border-color:rgba(212,169,64,0.5);color:#d4a940;">✨ AI 编辑</button>
                                                <button class="rt-npc-popup-library-btn menu_button" style="flex-shrink:0;font-size:12px;padding:4px 12px;white-space:nowrap;">📚 保存到资料库</button>
                                            </div>
                                        </div>
                                ...
                                <div style="border-top:2px solid rgba(212,169,64,0.15);padding-top:18px;">
                                    ...
                                    <div class="rt-npc-popup-ai-edit" style="display:none;flex-direction:column;gap:10px;">
                                        ...
                                        <textarea class="rt-npc-popup-ai-instructions" spellcheck="true" placeholder="描述您希望修改或迭代的内容… 例如‘让她的背景故事更悲剧一些’或‘添加与玩家角色的对抗关系’" style="width:100%;min-height:90px;box-sizing:border-box;background:var(--SmartThemeBlurTintColor, rgba(0,0,0,0.3));color:var(--SmartThemeBodyColor, inherit);border:1px solid rgba(212,169,64,0.5);border-radius:8px;padding:12px;font-family:inherit;font-size:13px;line-height:1.6;resize:vertical;"></textarea>
                                        <div style="display:flex;gap:8px;justify-content:flex-end;">
                                            <button class="rt-npc-popup-ai-cancel-btn menu_button" style="font-size:12px;padding:5px 14px;">取消</button>
                                        ...
                                        <div class="rt-npc-popup-ai-preview" style="display:none;flex-direction:column;gap:8px;">
                                            <div style="font-size:11px;font-weight:bold;color:rgba(255,255,255,0.4);text-transform:uppercase;letter-spacing:1.5px;">预览 — 应用前请先检查</div>
                            ...
                            `;
                                    ...
                                    const runNpcAiEdit = async () => {
                                        const instructions = aiInstructionsEl.value.trim();
                                        if (!instructions) { toastr['warning']('请描述您想要修改的内容。', 'AI 编辑'); return; }
                                        aiGenerateBtn.disabled = true; aiRegenBtn.disabled = true;
                                        aiGenerateBtn.textContent = '⏳ 生成中…';
                                        const curSettings = getSettings();
                                        const aiSettings = {
                                        ...
                                        try {
                                            ...
                                            toastr['error'](`AI 编辑失败：${String(err.message || err).substring(0, 120)}`, 'AI编辑');
                                        }
                                        aiGenerateBtn.disabled = false; aiRegenBtn.disabled = false;
                                        aiGenerateBtn.textContent = '✨ 生成';
                                    };
                                    ...
                                    aiApplyBtn.addEventListener('click', async () => {
                                        ...
                                        if (ok) {
                                            ...
                                            sectionsDiv.innerHTML = newHtml || `<div style="font-size:14px;color:var(--SmartThemeBodyColor, inherit);opacity:0.5;font-style:italic;padding:16px 0;">未找到结构化段落。</div>`;
                                            npcShowPane(viewPane);
                                            // @ts-ignore
                                            toastr.success('已通过 AI 更新 NPC 条目。', 'Lorebook Agent');
                                        } else {
                                            // @ts-ignore
                                            toastr.error('保存失败。', 'Lorebook Agent');
                                        }
                                    ...
                                    if (clearLogBtn) {
                                        clearLogBtn.addEventListener('click', () => {
                                            if (confirm('确定清除此 NPC 的关系历史记录吗？（此操作无法撤销）')) {
                                    ...
                                    saveBtn.addEventListener('click', async () => {
                                        ...
                                        if (ok) {
                                            ...
                                            toastr.success('条目已保存。', 'Lorebook Agent');
                                            const newHtml = renderSectionsHtml(item.content);
                                            sectionsDiv.innerHTML = newHtml || `<div style="font-size:14px;color:var(--SmartThemeBodyColor, inherit);opacity:0.5;font-style:italic;padding:16px 0;">未找到结构化段落。点击「编辑文本」以添加内容。</div>`;
                                            editPane.style.display = 'none';
                                            viewPane.style.display = 'block';
                                        } else {
                                            // @ts-ignore
                            ...
                            if (isLocBook) {
                                ...
                                openLocationDetailPopup = async (item, fullPath) => {
                                    ...
                                    const activeStyle = item.is_active
                                        ? 'background:rgba(0,255,170,0.12);color:#00ffaa;border:1px solid rgba(0,255,170,0.25);'
                                        : 'background:var(--SmartThemeBorderColor, rgba(128,128,128,0.1));color:var(--SmartThemeBodyColor, inherit);opacity:0.65;border:1px solid var(--SmartThemeBorderColor, rgba(128,128,128,0.2));';
                                    const activeLabel = item.is_active ? '● 已激活' : '○ 未激活';
                                    ...
                                    if (editBtn && viewPane && editPane && textarea) {
                                        ...
                                        saveBtn?.addEventListener('click', async () => {
                                            ...
                                            if (ok) {
                                                ...
                                                toastr.success('位置条目已保存。', 'Lorebook Agent');
                                                await refreshManifest();
                                            } else {
                                                toastr.error('保存失败。', 'Lorebook Agent');
                                            }
                            ...
                            if (useNpcCardView) {
                                ...
                                <div class="rt-npc-info">
                                    <div class="rt-npc-name">${escapeHtml(item.label)}${isDirty ? ' <span style="color:#ffa500; font-size:8px;" title="未保存的编辑">●</span>' : ''}</div>
                                    <div class="rt-npc-desc">${escapeHtml(desc)}</div>
                                    ...
                                    <div class="rt-npc-actions">
                                        <button class="rt-npc-action-btn rt-npc-view" data-id="${item.id}" title="查看 NPC 卡片"><i class="fa-solid fa-address-card"></i> 完整 NPC 卡片</button>
                                        <button class="rt-npc-action-btn rt-npc-library" data-id="${item.id}" title="保存到 NPC 图库"><i class="fa-solid fa-bookmark"></i></button>
                                        <button class="rt-npc-action-btn rt-npc-pin" data-id="${item.id}" title="${isPinned ? '已固定——对 Lorebook Agent 始终生效' : '固定——保持此条目永久生效'}" style="${isPinned ? 'color:#34a853;' : ''}"><i class="fa-solid fa-thumbtack"></i></button>
                                        <button class="rt-npc-action-btn rt-npc-edit" data-id="${item.id}" title="编辑条目"><i class="fa-solid fa-pen-to-square"></i></button>
                                        <button class="rt-npc-action-btn rt-npc-clean" data-id="${item.id}" title="清理条目"><i class="fa-solid fa-broom"></i></button>
                                        <button class="rt-npc-action-btn rt-npc-delete" data-id="${item.id}" title="删除条目"><i class="fa-solid fa-trash"></i></button>
                                    </div>
                                </div>
                            `;
                                    ...
                                    if (cleanBtn) cleanBtn.addEventListener('click', async (e) => {
                                        e.stopPropagation();
                                        if (isRouterRunning()) { toastr['warning']('Agent 正在运行中。'); return; }
                                    ...
                                    if (delBtn) delBtn.addEventListener('click', async (e) => {
                                        e.stopPropagation();
                                        if (confirm(`确认删除 NPC“${item.label}”？`)) {
                                            const ok = await deleteLorebookEntry(item.id);
                                            if (ok) {
                                                _dirtyEntries.delete(item.id);
                                                _openEntries.delete(item.id);
                                                ...
                                                await refreshManifest();
                                                toastr['success'](`已删除“${item.label}”`, 'NPCs');
                                            }
                                        }
                                    });
                                    ...
                                    if (portraitWrap) {
                                        ...
                                        portraitWrap.addEventListener('drop', async (e) => {
                                            ...
                                            try {
                                                ...
                                                toastr['success'](`已为 ${item.label} 应用立绘`, 'NPC Portrait');
                                                await refreshManifest();
                                                refreshRenderedView();
                                            } catch (err) {
                                                toastr['error']('立绘应用失败。', 'NPC Portrait');
                                            }
                                        });
                                    }
                                ...
                                const renderNode = (node, parentElement) => {
                                    ...
                                    if (node.item) {
                                        ...
                                        const statusTitle = isPinned
                                            ? '已置顶——始终启用'
                                            : (node.item.is_active ? '活跃（对智能体可见）' : '未激活');
                                        statusDotHtml = `<div style="width:5px; height:5px; border-radius:50%; background:${statusColor}; flex-shrink:0;" title="${statusTitle}"></div>`;
                                    } else if (!isDayNode) {
                                        statusDotHtml = `<div style="width:5px; height:5px; border-radius:50%; border:1px dashed rgba(255,255,255,0.25); box-sizing:border-box; flex-shrink:0;" title="虚拟父占位（条目尚未创建）"></div>`;
                                    }
                                    ...
                                    if (node.item) {
                                        ...
                                        tokensHtml = `<span style="font-size:8px; opacity:0.5; color:var(--rt-text-muted); margin-right:5px; flex-shrink:0; background:rgba(255,255,255,0.06); padding:1px 4px; border-radius:4px;" title="估算 Token">${entryTokens}t</span>`;
                                        if (isNpcBook) {
                                            viewNpcHtml = `<button class="rt-agent-entry-view-npc" data-id="${node.item.id}" style="background:rgba(212,169,64,0.12); border:1px solid rgba(212,169,64,0.35); border-radius:3px; color:#d4a940; cursor:pointer; font-size:10px; padding:1px 5px; flex-shrink:0; line-height:1.2;" title="查看 NPC CORE 卡"><i class="fa-solid fa-address-card"></i></button>`;
                                            viewNpcHtml += `<button class="rt-agent-entry-save-npc" data-id="${node.item.id}" style="background:none; border:none; color:var(--rt-text-muted); cursor:pointer; font-size:9px; padding:1px 3px; flex-shrink:0;" title="保存至 NPC 库"><i class="fa-solid fa-bookmark"></i></button>`;
                                        ...
                                        if (useLocImageView && getLocationPathFromNode) {
                                            ...
                                            locThumbHtml = `<div class="rt-loc-thumb-wrap" title="${imageMeta.src ? '管理地点图片' : '设置地点图片'}">${thumbInner}</div>`;
                                            viewLocHtml = `<button class="rt-agent-entry-view-loc" data-id="${node.item.id}" style="background:rgba(94,184,212,0.12); border:1px solid rgba(94,184,212,0.35); border-radius:3px; color:#5eb8d4; cursor:pointer; font-size:10px; padding:1px 5px; flex-shrink:0; line-height:1.2;" title="查看地点详情"><i class="fa-solid fa-map"></i></button>`;
                                        }
                                        cleanHtml = !isWorldBook ? `<button class="rt-agent-entry-clean" data-id="${node.item.id}" style="background:none; border:none; color:#e67e22; cursor:pointer; font-size:9px; padding:1px 3px; flex-shrink:0;" title="针对此词条执行定向清理"><i class="fa-solid fa-broom"></i></button>` : '';
                                        const isPinned = !!node.item.is_pinned;
                                        pinHtml = `<button class="rt-agent-entry-pin" data-id="${node.item.id}" style="background:none; border:none; color:${isPinned ? '#34a853' : 'var(--rt-text-muted)'}; cursor:pointer; font-size:9px; padding:1px 3px; flex-shrink:0; ${isPinned ? 'opacity:1;' : 'opacity:0.55;'}" title="${isPinned ? '已置顶——对智能体永久启用' : '置顶——使此词条永久启用'}"><i class="fa-solid fa-thumbtack"></i></button>`;
                                        editHtml = `<button class="rt-agent-entry-edit" data-id="${node.item.id}" style="background:none; border:none; color:var(--rt-accent); cursor:pointer; font-size:9px; padding:1px 3px; flex-shrink:0;" title="编辑此词条"><i class="fa-solid fa-pen-to-square"></i></button>`;
                                        deleteHtml = `<button class="rt-agent-entry-delete" data-id="${node.item.id}" style="background:none; border:none; color:var(--rt-text-muted); cursor:pointer; font-size:9px; padding:1px 3px; flex-shrink:0;" title="删除词条"><i class="fa-solid fa-trash"></i></button>`;
                                    }

                                    if (dungeonRealityEnabled && isLocBook) {
                                        const isLocationRoot = !(node.parent && node.parent.name);
                                        if (node.item?.has_dungeon_map) {
                                            dungeonMapBadgeHtml = `<span class="rt-dungeon-map-actions"><button class="rt-dungeon-map-badge" type="button" title="查看此根地点附带的私人地城地图（alpha）"><i class="fa-solid fa-map-location-dot"></i> 地图</button><button class="rt-dungeon-map-delete" type="button" title="从此地点移除私人地图（保留 CORE）"><i class="fa-solid fa-xmark"></i></button></span>`;
                                        } else if (isLocationRoot) {
                                            dungeonMapBadgeHtml = `<span class="rt-dungeon-map-actions"><button class="rt-dungeon-map-create" type="button" title="为此地点根创建私人地图"><i class="fa-solid fa-plus"></i> 地图</button></span>`;
                                        }
                                    }
                            ...
                            ${viewLocHtml}
                            ${statusDotHtml}
                            <span class="rt-agent-entry-label-span" style="${labelStyle}">${escapeHtml(node.name)}${isDirty ? ' <span style="color:#ffa500; font-size:8px;" title="未保存的编辑">●</span>' : ''}</span>
                            ${relStatsHtml}
                            ${dungeonMapBadgeHtml}
                            ${tokensHtml}
                            ${pinHtml}
                        ...
                        `;
                                    ...
                                    if (dungeonMapDeleteBtn && node.item) {
                                        dungeonMapDeleteBtn.addEventListener('click', async (e) => {
                                            ...
                                            if (!confirm(`从“${label}”移除私人地图？\n\n地点条目与 CORE 保留，此场地的地图演进历史将被清除。`)) return;
                                            const result = await deleteDungeonMapFromLocationEntry(node.item.id);
                                            if (result?.ok) {
                                                await refreshManifest();
                                                toastr.success(`已从“${label}”移除地图。`, 'Lorebook Agent');
                                            } else {
                                                toastr.error(result?.error || '无法移除该地图。', 'Lorebook Agent');
                                            }
                                        });
                                    }
                                        ...
                                        dungeonMapCreateBtn.addEventListener('click', async (e) => {
                                            ...
                                            try {
                                                ...
                                                if (choice?.ok) {
                                                    ...
                                                    toastr.error(choice?.error || '无法创建该地图。', 'Persistent Maps');
                                                }
                                            } finally {
                                    ...
                                    if (locThumbWrap && node.item && locFullPath) {
                                        ...
                                        locThumbWrap.addEventListener('drop', async (ev) => {
                                            ...
                                            try {
                                                ...
                                                toastr.success(`已为 ${locFullPath} 应用地点图片`, 'Location Image');
                                                await refreshManifest();
                                            } catch (err) {
                                                toastr.error('地点图片应用失败。', 'Location Image');
                                            }
                                        });
                                    }
                        ...
                        const existingLoading = list.querySelector('#rt-agent-manifest-loading');
                        if (existingLoading) existingLoading.innerHTML = '<span style="color:#ff5555;">加载清单失败。</span>';
                    }
                };
                ...
                list.innerHTML = '<div style="text-align: center; color: #ff5555; font-size: 0.769em; padding: 10px;">渲染玩家角色失败。</div>';
            }
        };
        ...
        const createNpcFromCharCard = async (charCard, bookName, adaptedContent = null) => {
            ...
            for (const [, entry] of Object.entries(bookData.entries)) {
                ...
                if (entryLabel === cleanLabel) {
                    toastr['warning'](`NPC「${name}」已存在于该世界书中。`, 'NPC 导入');
                    return false;
                }
            }
            ...
            if (!saveRes.ok) {
                toastr['error']('NPC 条目保存失败。', 'NPC 导入');
                return false;
            }
...
- Do NOT use the "|\'" character inside the Description; use newlines to separate internal sections.`;
            ...
            try {
                ...
                toastr['error'](`AI 审查失败：${String(err.message || err).substring(0, 120)}`, 'NPC 导入');
                return null;
            }
        };
...
- Output ONLY this single [[NPC: ...]] string. No preamble, no explanation, no other tags.`;
            ...
            try {
                ...
                toastr['error'](`AI 适配失败：${String(err.message || err).substring(0, 120)}`, 'NPC 导入');
                return null;
            }
        };
...
${namingRule}`;
            ...
            try {
                ...
                toastr['error'](`NPC 生成失败：${String(err.message || err).substring(0, 120)}`, 'NPC 创建器');
                return null;
            }
        };
        ...
        const openNpcCreatorDialog = async (bookName, prefix) => {
            ...
            try {
                ...
                if (err.name === 'AbortError') {
                    toastr['error']('角色列表请求超时，请重试。', 'NPC 导入');
                } else {
                    toastr['error']('无法加载角色卡。', 'NPC 导入');
                }
                return;
            }
            ...
            header.innerHTML = `<h3>📚 NPC/PC 管理器</h3>`;
            ...
            const tabDefs = [
                ...
                { id: 'card', label: '🗂️ 从角色卡导入' },
                { id: 'freeform', label: '✍️ 自由创建' },
                { id: 'archetype', label: '🎭 原型生成' },
            ];
            const tabBtns = {};
            const tabPanels = {};
            ...
            const showNpcPreviewAndAdd = async (generatedTag, defaultName, toastLabel, originalAvatar = null, portraitSrc = null) => {
                ...
                const warningHtml = isDuplicate
                    ? `<div style="font-size:0.8em;color:#ff5555;margin-bottom:8px;font-weight:bold;background:rgba(255,0,0,0.1);padding:6px;border-radius:4px;border:1px solid rgba(255,0,0,0.2);">⚠️ 名为 "${escapeHtml(nameToAdd)}" 的 NPC 已存在！请在 [[NPC: Name | ...]] 中修改名称后再添加。</div>`
                    : `<div style="font-size:0.8em;opacity:0.6;margin-bottom:8px;">请检查 AI 生成的内容，如有需要可编辑，然后确认。</div>`;

                const previewHtml = `<div style="padding:10px;min-width:320px;max-width:520px;">
                    <b style="display:block;margin-bottom:8px;">✨ 已生成 NPC — ${escapeHtml(nameToAdd)}</b>
                    ${warningHtml}
                ...
                const result = await ctx.callGenericPopup(previewHtml, ctx.POPUP_TYPE?.CONFIRM ?? 1, '', {
                    okButton: '✅ 添加 NPC', cancelButton: '取消', wide: false,
                });
                if (result) {
                    ...
                    if (finalBookData && finalBookData.entries) {
                        ...
                        for (const [, entry] of Object.entries(finalBookData.entries)) {
                            const entryLabel = (entry.comment || '').replace(/^\[.*?\]\s*/i, '').toLowerCase().trim();
                            if (entryLabel === cleanLabel) {
                                toastr['warning'](`NPC "${finalName}" 已存在，无法添加重复项。`, toastLabel);
                    ...
                    if (ok) {
                        toastr['success'](`已将 "${finalName}" 添加为 NPC。`, toastLabel);
                        dismissOverlay();
                        await refreshManifest();
                    }
                }
            ...
            // ── Tab 1: Import from Character Card ──────────────────────────
            {
                ...
                searchInput.placeholder = '🔍 按名称搜索角色...';
                ...
                const renderList = () => {
                    ...
                    if (filtered.length === 0) {
                        listContainer.innerHTML = '<div class="rt-charpicker-empty">没有符合搜索条件的角色。</div>';
                        return;
                    }
                    ...
                    for (const char of visible) {
                        ...
                        descEl.textContent = (char.description || char.personality || '无描述').substring(0, 120);
                        ...
                        directBtn.textContent = '+ 原样添加';
                        directBtn.title = getSettings().npcAddAsIsMode === 'ai_review'
                            ? 'AI 审查模式：添加前将角色卡发送给 AI 进行最小逻辑审查（仅处理时代/世界观冲突）。'
                            : '逐字模式：按原样将角色卡内容包入 [CORE][/CORE]，不涉及 AI。';
                        directBtn.addEventListener('click', async () => {
                            const mode = getSettings().npcAddAsIsMode ?? 'ai_review';
                            directBtn.disabled = true;
                            directBtn.textContent = mode === 'ai_review' ? '⏳ 审查中...' : '⏳ 添加中...';
                            try {
                                if (mode === 'ai_review') {
                                    // Minimal AI review pass — fix only world/era impossibilities
                                    const reviewed = await minimalReviewNpcWithAI(char);
                                    if (!reviewed) { directBtn.disabled = false; directBtn.textContent = '+ 原样添加'; return; }
                                    await showNpcPreviewAndAdd(reviewed, char.name, 'NPC 创建器', char.avatar);
                                } else {
                                    // Literal — wrap verbatim, no AI
                                    const ok = await createNpcFromCharCard(char, bookName);
                                    if (ok) {
                                        toastr['success'](`已将 "${char.name}" 添加为 NPC。`, 'NPC 创建器');
                                        dismissOverlay();
                                        await refreshManifest();
                                    }
                                }
                            } catch (err) {
                                toastr['error'](`失败：${String(err.message || err).substring(0, 100)}`, 'NPC 创建器');
                            } finally {
                        ...
                        aiBtn.textContent = '🤖 融入剧情';
                        aiBtn.addEventListener('click', async () => {
                            aiBtn.disabled = true;
                            aiBtn.textContent = '⏳ 适配中...';
                            try {
                                const adapted = await adaptNpcWithAI(char);
                                if (!adapted) { aiBtn.disabled = false; aiBtn.textContent = '🤖 融入剧情'; return; }
                                await showNpcPreviewAndAdd(adapted, char.name, 'NPC Creator', char.avatar);
                            } catch (err) {
                                toastr['error'](`适配失败：${String(err.message || err).substring(0, 100)}`, 'NPC Creator');
                            } finally {
                    ...
                    if (visible.length < filtered.length) {
                        ...
                        loadMore.textContent = `显示更多（${visible.length}/${filtered.length}）`;
            ...
            // ── Tab 2: Freeform Description ────────────────────────────────
            {
                ...
                hintEl.textContent = '用你自己的话描述这个 NPC。AI 会将描述扩展为一条符合当前战役设定的完整世界书（lorebook）词条。';
            ...
            // ── Tab 3: Archetype Generator ─────────────────────────────────
            {
                ...
                hintEl.textContent = '在下方选择一个剧情定位，或输入自定义定位。AI 将结合当前战役背景生成一个合适的 NPC。';
                ...
                customLabel.textContent = '自定义原型 / 角色 *';
                customLabel.style.display = 'none';

                const customInput = document.createElement('input');
                customInput.className = 'rt-npc-form-input';
                customInput.type = 'text';
                customInput.placeholder = '例如：导师、酒保、公会会长...';
                ...
                genBtn.addEventListener('click', async () => {
                    ...
                    if (!role) { toastr['warning']('请先选择或输入一个原型/角色。', 'NPC Creator'); return; }
                    genBtn.disabled = true;
                    genBtn.textContent = '⏳ 生成中...';
                    try {
            ...
            // ── Tab 4: NPC Library ─────────────────────────────────────────
            {
                ...
                importBtn.title = '导入 .mnpc.json NPC 包（肖像已嵌入文件中）';

                const hint = document.createElement('div');
                hint.className = 'rt-npc-library-hint';
                hint.textContent = '身份卡与角色无关。可直接添加，或作为PC游玩；适配到故事会将其适配为当前战役中的NPC。添加到队伍会向状态追踪器发送加入信息。';
                ...
                const installLibraryCardAsPlayerCharacter = async (rec) => {
                    ...
                    if (!chatId) {
                        toastr['warning']('当前没有活动的聊天，无法附加玩家卡。', 'Library');
                        return false;
                    }
                    const s = getSettings();
                    ...
                    if (existing?.name && String(existing.name) !== String(rec.name)) {
                        if (!confirm(`将玩家卡"${existing.name}"替换为"${rec.name}"？`)) return false;
                    } else if (existing?.name && String(existing.name) === String(rec.name)) {
                        if (!confirm(`"${rec.name}"已经是玩家卡。要从库中覆盖它吗？`)) return false;
                    }
                    ...
                    toastr['info'](`正在将"${rec.name}"设为玩家卡并更新[CHARACTER]…`, 'Library');
                    ...
                    if (result?.status !== 'busy') {
                        toastr['warning'](`玩家卡已设置。状态追踪器未更新[CHARACTER]：${result?.message || '无更改'}。`, 'Library');
                    }
                    return true;
                };
                ...
                const renderLibraryList = () => {
                    ...
                    if (records.length === 0) {
                        listContainer.innerHTML = '<div class="rt-charpicker-empty">库为空。在战役记录中为NPC添加书签，或导入包。</div>';
                        return;
                    }
                    if (filtered.length === 0) {
                        listContainer.innerHTML = '<div class="rt-charpicker-empty">没有与搜索匹配的库NPC。</div>';
                        return;
                    }
                    ...
                    for (const rec of filtered) {
                        ...
                        const nameEl = document.createElement('div');
                        nameEl.className = 'rt-charpicker-name';
                        nameEl.textContent = rec.name || '未命名';
                        const descEl = document.createElement('div');
                        descEl.className = 'rt-charpicker-desc';
                        descEl.textContent = getCardLibraryBlurb(rec.content, substituteDisplayMacros) || '无描述';
                        infoDiv.appendChild(nameEl);
                        infoDiv.appendChild(descEl);
                        ...
                        viewBtn.innerHTML = '<i class="fa-solid fa-address-card"></i> 完整卡片';
                        viewBtn.title = '查看完整库卡片（CORE身份与肖像）';
                        ...
                        addBtn.textContent = '+ 原样添加';
                        addBtn.title = '将此身份连同肖像作为NPC复制到当前故事中。';
                        addBtn.addEventListener('click', async (e) => {
                            ...
                            try {
                                ...
                                if (ok) {
                                    toastr['success'](`已将"${rec.name}"添加为NPC。`, 'Library');
                                    dismissOverlay();
                                    await refreshManifest();
                                }
                            } catch (err) {
                                toastr['error'](`失败：${String(err.message || err).substring(0, 100)}`, 'Library');
                            } finally {
                        ...
                        aiBtn.addEventListener('click', async (e) => {
                            ...
                            try {
                                ...
                                if (!adapted) { aiBtn.disabled = false; aiBtn.textContent = '🤖 融入故事'; return; }
                                await showNpcPreviewAndAdd(adapted, rec.name, 'NPC Library', null, rec.portraitPath || '');
                            } catch (err) {
                                toastr['error'](`适配失败：${String(err.message || err).substring(0, 100)}`, 'NPC Library');
                            } finally {
                                aiBtn.disabled = false;
                                aiBtn.textContent = '🤖 融入故事';
                            }
                        ...
                        partyBtn.textContent = '+ 加入队伍';
                        partyBtn.title = '发送此卡片给 State Tracker，使其加入 [PARTY]。';
                        partyBtn.addEventListener('click', async (e) => {
                            e.stopPropagation();
                            partyBtn.disabled = true;
                            partyBtn.textContent = '⏳ 添加中…';
                            try {
                                toastr['info'](`正在通过 State Tracker 将“${rec.name}”加入队伍…`, 'Library');
                                ...
                                if (result?.success && result.changed) {
                                    ...
                                    toastr['error'](result?.message || 'State Tracker 未能添加该队伍成员。', 'Library');
                                }
                            } catch (err) {
                                toastr['error'](`失败：${String(err.message || err).substring(0, 100)}`, 'Library');
                            } finally {
                        ...
                        pcBtn.textContent = '▶ 扮演 PC';
                        pcBtn.title = '将此身份设为 Player Card，并请 State Tracker 交换 [CHARACTER]。';
                        pcBtn.addEventListener('click', async (e) => {
                            e.stopPropagation();
                            pcBtn.disabled = true;
                            pcBtn.textContent = '⏳ 设置中…';
                            try {
                        ...
                        exportBtn.title = '导出为 .mnpc.json（包含立绘）';
                        exportBtn.addEventListener('click', async (e) => {
                            e.stopPropagation();
                            try {
                                const hadPortrait = await exportNpcToFile(rec);
                                if (hadPortrait) {
                                    toastr['success'](`已导出“${rec.name}”（含立绘）。`, 'NPC Library');
                                } else if (rec.portraitPath) {
                                    toastr['warning'](`已导出“${rec.name}”（不含立绘，图片无法读取）。`, 'NPC Library');
                                } else {
                                    toastr['success'](`已导出“${rec.name}”。`, 'NPC Library');
                                }
                            } catch (err) {
                                toastr['error'](`导出失败：${String(err.message || err).substring(0, 100)}`, 'NPC Library');
                            }
                        });
                        ...
                        delBtn.addEventListener('click', async (e) => {
                            e.stopPropagation();
                            if (!confirm(`从图库中移除“${rec.name}”？这不会将其从当前故事中删除。`)) return;
                            try {
                                await deleteNpcFromLibrary(getSettings(), rec.id);
                                toastr['info'](`已从图库中移除“${rec.name}”。`, 'NPC Library');
                                renderLibraryList();
                            } catch (err) {
                                toastr['error'](`删除失败：${String(err.message || err).substring(0, 100)}`, 'NPC Library');
                            }
                        });
                ...
                fileInput.addEventListener('change', async () => {
                    ...
                    if (imported > 0) {
                        toastr['success'](`已导入 ${imported} 个 NPC。`, 'NPC Library');
                        renderLibraryList();
                    }
                });
...
if (activateBooksBtn) activateBooksBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                toastr['success'](`已激活 ${count} 本战役资料库。`);
                await refreshManifest('manual-button');
            } catch (e) {
                toastr['error']('激活战役资料库失败。');
            } finally {
        ...
        // 初始清单加载延迟到 Lorebook Agent 面板打开时进行。
        // refreshManifest() 由 agent 开关处理程序和手动刷新按钮调用。

        /**
         * 共享插槽栏：[[TAG: 名称 | [插槽 ×]... + | 关键词]]
         * 中间标签可编辑、可移除；+ 按钮用于添加新插槽。
         * 每当格式字符串变化时调用 onFormatChange(newFmt)。
         */
        const buildSlotBar = (tagName, format, onFormatChange) => {
            ...
            const renderBar = (currentFmt) => {
                ...
                middles.forEach((label, idx) => {
                    ...
                    inp.type = 'text';
                    inp.value = label;
                    inp.title = '重命名此插槽——AI 将根据其名称填写此部分';
                ...
                chipLast.title = '固定——始终为逗号分隔的搜索关键词';
        ...
        const renderAgentModules = () => {
            ...
            Object.entries(s.routerModules || {}).forEach(([id, config]) => {
                ...
                // 显示此模块实际写入的资料库名称（例如“Locations”）以及
                // 其提示标记（LOC），以便命名与用户自己的资料库保持一致。
                const bookCategory = MODULE_BOOK_CATEGORY?.[id];
                const moduleLabel = bookCategory
                    ? `${bookCategory} <span style="opacity:0.6; font-weight:normal;" title="Lorebook Agent 输出中使用的提示标记——实际资料库名为 ..._${bookCategory}">(${config.tag})</span>`
                    : config.tag;
                header.innerHTML = `
                        ...
                        <button class="rt-agent-module-reset" style="background:transparent; border:none; color:var(--rt-accent); cursor:pointer; font-size:0.692em; padding:0 4px; opacity:0.5;" title="将插槽和指令重置为默认值"><i class="fa-solid fa-arrow-rotate-left"></i></button>
                    `;
                ...
                header.querySelector('.rt-agent-module-reset').addEventListener('click', () => {
                    if (confirm(`将 ${id.toUpperCase()} 模块的插槽和指令重置为默认值？`)) {
                        const st = getSettings();
        ...
        const renderAgentCustomTags = () => {
            ...
            (s.routerCustomTags || []).forEach((tag, idx) => {
                ...
                inst.rows = 2;
                inst.placeholder = '此标签的指令…';
                inst.title = '指令文本 — 指导每个槽位应写入的内容';
                inst.style.cssText = 'width:100%; background:rgba(0,0,0,0.3); color:var(--rt-text); border:1px solid rgba(255,255,255,0.1); border-radius:3px; font-size:0.692em; padding:2px 4px; box-sizing:border-box; margin-top:2px; resize:vertical !important; min-height:38px; font-family:inherit;';
                inst.addEventListener('change', () => {
                    const st = getSettings();
        ...
        if (agentPauseBtn) {
            agentPauseBtn.addEventListener('click', (e) => {
                ...
                if (agentPauseBanner) agentPauseBanner.textContent = s.routerPaused ? '代理已暂停' : '';
            });
        }
        ...
        if (manualRunBtn && researchDropdown) {
            ...
            const runManualMapUpdater = async () => {
                ...
                if (skipped === 'location_mapping_off' || skipped === 'dungeon_reality_off') toastr.warning('持久化地图已关闭。', 'Map Updater');
                else if (skipped === 'no_active_map') toastr.warning('没有活动的地下城或聚落地图。', 'Map Updater');
                else if (skipped === 'disabled') toastr.warning('地图更新器已禁用。', 'Map Updater');
                else if (skipped === 'busy') toastr.warning('已有代理正在运行。', 'Map Updater');
                else if (skipped === 'stopped') toastr['info']('已停止。', 'Map Updater');
                else if (result?.ok && result?.noop) toastr['info']('没有持久性变化。', 'Map Updater');
                else if (result?.ok) toastr['success']('占用更新已应用。', 'Map Updater');
                else toastr.error('无法应用有效的占用更新。', 'Map Updater');
            };
            const runManualMapEvolution = async () => {
                if (agentsBusy()) {
                    toastr.warning('已有代理正在运行。', 'Map Evolution');
                    return;
                }
                ...
                if (!siteRoots) return;
                if (!siteRoots.length) {
                    toastr.warning('请至少勾选一个已映射地点。', 'Map Evolution');
                    return;
                }
                toastr['info']('正在启动地图演化流程…');
                updateAgentStatusIndicator(isRouterRunning());
                const result = await runMapEvolutionPass({ trigger: 'manual', isManual: true, siteRoots });
                updateAgentStatusIndicator(isRouterRunning());
                const skipped = result?.skipped;
                if (skipped === 'location_mapping_off' || skipped === 'dungeon_reality_off') toastr.warning('持久化地图已关闭。', 'Map Evolution');
                else if (skipped === 'no_maps' || skipped === 'no_active_map' || skipped === 'no_matching_sites') toastr.warning('没有可演化的已映射地点。', 'Map Evolution');
                else if (skipped === 'disabled') toastr.warning('地图演化已禁用。', 'Map Evolution');
                else if (skipped === 'busy') toastr.warning('已有代理正在运行。', 'Map Evolution');
                else if (skipped === 'stopped') toastr['info']('已停止。', 'Map Evolution');
                else if (result?.baseline) toastr['info']('已记录间隔基线。间隔过后将触发演化。', 'Map Evolution');
                else if (result?.ok && result?.applied === 0) toastr['info']('没有持久性变化。', 'Map Evolution');
                else if (result?.ok) toastr['success']('地图演化已应用。', 'Map Evolution');
                else toastr.error('无法应用有效的演化更新。', 'Map Evolution');
            };
        ...
        if (cleanupBroomBtn && cleanupDropdown) {
            ...
            if (cleanupRunBtn) {
                cleanupRunBtn.addEventListener('click', async (e) => {
                    ...
                    const promptHtml = `
                            <div style="text-align: left; font-size: 0.9em; line-height: 1.4;">
                                <p>您即将触发一次 <b>全局清理模式</b> 运行，以整合所有臃肿的设定条目（&gt;${threshold} tokens）。</p>
                                <p style="margin-top: 8px;">为全局压缩输入自定义要求（例如 <i>"保留背景设定细节，但精简任务状态"</i>）：</p>
                                <textarea id="rt-global-clean-instructions" style="width: 100%; height: 60px; background: rgba(0,0,0,0.3); border: 1px solid rgba(255,255,255,0.2); color: white; border-radius: 4px; padding: 5px; font-size: 12px; box-sizing: border-box; resize: none; margin-top: 5px;" placeholder="留空以进行标准清理…"></textarea>
                            </div>
                        `;

                    const choice = await Popup.show.confirm('🧹 全局书库清理', promptHtml, {
                        okButton: '清理全部臃肿条目',
                        cancelButton: '取消'
                    });
                    ...
                    if (choice) {
                        ...
                        toastr['info']('正在启动书库清理模式…', 'Lorebook Agent');
        ...
        if (detachBtn) {
            ...
            const applyDetachedState = () => {
                ...
                if (isDetached()) {
                    ...
                    detachBtn.innerHTML = '↓';
                    detachBtn.title = '重新挂接 Lorebook Agent';
                    ...
                    detachBtn.innerHTML = '⧉';
                    detachBtn.title = '分离 Lorebook Agent';
    ...
    textarea.addEventListener('input', (e) => {
        ...
        panel.querySelector('#rpg-tracker-count').textContent = `~${Math.round(newText.length / 2.62)} 个 Token`;
    ...
    panel.querySelector('#rpg-tracker-portraits-menu-btn').addEventListener('click', async (e) => {
        ...
        const popupContent = `<div style="padding:10px;min-width:260px;">
            <b style="display:block;margin-bottom:8px;">🖼️ AI 立绘操作</b>
            <div style="font-size:0.85em;opacity:0.8;margin-bottom:12px;">选择要执行的操作，管理或生成当前活跃实体的立绘。</div>
        </div>`;
        ...
        const popupOpts = {
            ...
            wide: false,
            customButtons: [
                { text: '✨ 自动生成队伍立绘', result: 1001, classes: ['menu_button'] },
                { text: '😈 自动生成敌人立绘', result: 1003, classes: ['menu_button'] },
                { text: '🗑 清除全部立绘', result: 1002, classes: ['menu_button', 'danger'] },
            ],
        };
    ...
    panel.querySelector('#rpg-tracker-close-btn').addEventListener('click', () => {
        ...
        if (settings.closeCount === 1 || settings.closeCount % 10 === 0) {
            toastr['info']('追踪器已隐藏。你可以随时从扩展（魔杖）菜单重新打开它。', 'RPG Tracker');
        }
        saveSettings();
    });
    ...
    const manualUpdate = async (type = 'regular') => {
        ...
        if (type === 'regular') {
            ...
            const count = await Popup.show.input("RPG Tracker", "需要回溯解析多少条消息？", "5");
            if (!count || isNaN(parseInt(count))) return;
            customLookbackN = parseInt(count);
            narrative = getNarrativeBlocks(chat, customLookbackN);
        }

        if (type !== 'full' && !narrative) return toastr['info']("没有可解析的助手消息。", "RPG Tracker");

        toastr['info'](isFullAudit ? "正在触发完整上下文审计…" : "正在触发手动状态更新…", "RPG Tracker");
    ...
    updateMenu.innerHTML = `
            <div class="rt-menu-item" id="rt-update-regular"><b>常规更新</b><small>自上次用户消息以来</small></div>
            <div class="rt-menu-item" id="rt-update-custom"><b>回溯更新</b><small>最近 N 条消息</small></div>
            <div class="rt-menu-item" id="rt-update-full"><b>完整上下文审计</b><small>重新审查全部历史</small></div>
        `;
    ...
    overflowMenu.innerHTML = `
        <div class="rt-overflow-section-header">操作</div>
        <div class="rt-overflow-item" id="rt-ov-enable"><span class="rt-ov-icon">⏻</span><span id="rt-ov-enable-label">启用 / 禁用</span></div>
        <div class="rt-overflow-item" id="rt-ov-pause"><span class="rt-ov-icon">⏸</span><span id="rt-ov-pause-label">暂停追踪器</span></div>
        <div class="rt-overflow-item" id="rt-ov-portraits"><span class="rt-ov-icon">🖼️</span><span>立绘操作</span></div>
        <div class="rt-overflow-section-header">更新状态</div>
        <div class="rt-overflow-item" id="rt-ov-upd-regular"><span class="rt-ov-icon">🔄</span><span>常规更新</span><small>自上次用户消息以来</small></div>
        <div class="rt-overflow-item" id="rt-ov-upd-custom"><span class="rt-ov-icon">🔄</span><span>回溯更新</span><small>最近 N 条消息</small></div>
        <div class="rt-overflow-item" id="rt-ov-upd-full"><span class="rt-ov-icon">🔄</span><span>完整上下文审计</span><small>重新审查全部历史</small></div>
    `;
    ...
    overflowBtn.addEventListener('click', (e) => {
        ...
        if (!isVisible) {
            ...
            if (enableLabel) enableLabel.textContent = s.enabled ? '禁用框架' : '启用框架';
            const pauseLabel = overflowMenu.querySelector('#rt-ov-pause-label');
            if (pauseLabel) pauseLabel.textContent = s.trackerPaused ? '恢复追踪器' : '暂停追踪器';
    ...
    panel.querySelector('#rpg-tracker-nav-label').addEventListener('click', () => {
        ...
        toastr['success']('历史状态已恢复为当前状态。', 'RPG Tracker');
    });

    // Clear memo button
    panel.querySelector('#rpg-tracker-memo-clear').addEventListener('click', () => {
        if (confirm("确定要清除记忆历史并重置追踪器吗？")) {
            ...
            toastr['success']("RPG Tracker 逻辑已清除。", "RPG Tracker");
        }
    });
