...
describe('Map Evolution', () => {
    ...
    it('keeps Evolution sequential, occupancy-separate, and wired through the pipeline', () => {
        ...
        expect(settingsMarkup).toContain('使用 Map Evolution 连接');
        ...
        expect(settingsMarkup).toContain('高动态 — 战术性且即将可发现');
        expect(settingsMarkup).toContain('标准 — 与后台地图行为相同');
        ...
        expect(settingsMarkup).toContain('<b>每地图间隔</b>');
        ...
        expect(settingsMarkup).toContain('报告不会立即触发地图扩散');
        ...
        expect(settingsMarkup).toContain('全部已映射地点（每轮 N 张）');
        expect(settingsMarkup).toContain('剩余到期地图会排队等待后续回合');
        ...
        expect(settingsMarkup).toContain('<b style="font-size:0.9em; flex:1;">已映射地点</b>');
        expect(settingsMarkup).toContain('<b>立即运行</b> 即勾选框');
        expect(settingsMarkup).toContain('该勾选不会启用或禁用单张地图的计时器');
        ...
        expect(settingsMarkup).toContain('Map Evolution 记忆');
        expect(settingsMarkup).toContain('转移给 <b>叙述者</b>');
        ...
        expect(debug).toContain('No Evolution pass to undo. Run one first.');
        ...
        expect(debugUi).toContain('已关闭线程令牌');
        expect(debugUi).toContain('存储的演化记忆');
        expect(debugUi).toContain('data-debug="threads-json"');
        expect(debugUi).toContain('data-debug="backlog-json"');
        expect(debugUi).toContain('Evolution 读取到的因果线程');
        expect(debugUi).toContain('资产轨迹');
